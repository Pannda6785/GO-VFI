...
def project_mask_to_patch_grid(
    ...
    resized = F.interpolate(mask.float(), size=grid_hw, mode="nearest")
    flat = resized.flatten(2).squeeze(1)
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        return out * query_mask.unsqueeze(-1).float()
...
class PairwiseDifferenceModel(nn.Module):
    ...
    def forward(
        ...
        patch_mask1 = project_mask_to_patch_grid(mask1, grid1, self.mask_threshold)
        patch_mask2 = project_mask_to_patch_grid(mask2, grid2, self.mask_threshold)
        valid = patch_mask1.any(dim=1) & patch_mask2.any(dim=1)

        if not valid.any():
            empty = tokens1.new_empty((0,))
            empty_tokens = tokens1.new_empty((0, tokens1.shape[1], self.proj.out_features))
            empty_mask = patch_mask1.new_empty((0, patch_mask1.shape[1]))
            empty_embed = tokens1.new_empty((0, self.proj.out_features))
            return ModelOutput(
                logits=empty,
                prob=empty,
                tokens1=empty_tokens,
                tokens2=empty_tokens,
                mask1=empty_mask,
                mask2=empty_mask,
                embedding1=empty_embed,
                embedding2=empty_embed,
                valid_indices=valid.nonzero(as_tuple=False).squeeze(-1),
            )

        tokens1 = tokens1[valid]
        tokens2 = tokens2[valid]
        patch_mask1 = patch_mask1[valid]
        patch_mask2 = patch_mask2[valid]

        tokens1 = self.proj(tokens1)
        tokens2 = self.proj(tokens2)
        tokens1 = tokens1 * patch_mask1.unsqueeze(-1).float()
        tokens2 = tokens2 * patch_mask2.unsqueeze(-1).float()

        attended1 = self.cross_12(tokens1, tokens2, patch_mask1, patch_mask2)
        attended2 = self.cross_21(tokens2, tokens1, patch_mask2, patch_mask1)

        embedding1 = masked_mean_pool(attended1, patch_mask1)
        embedding2 = masked_mean_pool(attended2, patch_mask2)
        relation = torch.cat(
